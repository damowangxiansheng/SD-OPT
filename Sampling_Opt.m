...
stepy = 400; % number of steps of b0
kv_min = -2; % min. value for b0
kv_max = 2; % max. value for b0
kr_min = -5; % min. value for delta
kr_max = 8; % max. value for delta
options.disp = 0; % options for command eigs
% SEMI-DISCRETIZATION
p = tau_s/h; % period resolution by the sampling part (not the constant part)
%h=T/p; % discretization step
r = floor(tau/h+1/2)+1; % delay resolution
r1 = r-p;
%% Matrix definition
Ai = A1;dim = length(A1);
%%
...
eig_m(x,y) = max(abs(eig(Phi))); % by eig
end
...
% hold on;
figure;contourf(kv_m,kr_m,eig_m,[linspace(1,0,100)],'EdgeColor','none');
colormap(gray);colorbar;
contour(kv_m,kr_m,eig_m,[1, 1],'k--');

% Find the minmum eigenvalue
min_eig = min(eig_m,[],'all');
[x_loc,y_loc]=find(eig_m == min(eig_m,[],'all'));
%SD_zeta = log(min_eig^2)/tau/2
SD_zeta = log(min_eig)/((p+1)*h)
min_kr = kr_m(1,y_loc);
min_kv = kv_m(x_loc,1);
%hold on;scatter(min_kv,min_kr,'*r');
xlabel('K_{Pv}'); ylabel('K_{Pr}');
hold on;
